...
function reduce(array, callback, initialValue) {
    ...
    {
        res=callback(initialValue,array[i]);
        if(res===true)
            return true;
    }
    return false;
...
// console.log(reduce(nums1, add, 0)); 
...
/* implement intersection
 function intersection(arrays) {
    ...
    for(let i=0;i<arrays[0].length;i++)
    {
        ...
        {
            if(reduce(arrays[j],compare,arrays[0][i])&&j===(arrays.length-1))
                ans.push(arrays[0][i]);
            else if(reduce(arrays[j],compare,arrays[0][i]))
                continue;
            else
                break;
        }
...
function union(arrays) {
    return arrays.reduce((accumulator,current)=>{
        const elements= current.filter(e=>!accumulator.includes(e));
...
console.log(multiMap(['catfood', 'glue', 'beer'],
 [function(str) { return str.toUpperCase(); },
  function(str) { return str[0].toUpperCase() +
...
function majority(array, callback) {
  ...
  array.forEach(e=>{
    if(callback(e))
      countTrue++;})
  return countTrue>array.length-countTrue;
}
...
function rating(arrOfFuncs, value) {
  ...
  return (countTrue/(arrOfFuncs.length))*100
}
